...
ODBCConnection* PGAtomStorage::get_conn()
{
	return conn_pool.pop();
}

/// Put an ODBC connection back into the pool.
void PGAtomStorage::put_conn(ODBCConnection* db_conn)
{
...
bool PGAtomStorage::idExists(const char * buff)
{
 ...
 */
void PGAtomStorage::storeOutgoing(AtomPtr atom, Handle h)
{
 ...
 */
bool PGAtomStorage::tvExists(int tvid)
{
	char buff[BUFSZ];
 ...
 * avoid having to look it up.
 */
int PGAtomStorage::storeTruthValue(AtomPtr atom, Handle h)
{
	int notfirst = 0;
	std::string cols;
	std::string vals;
 ...
 */
int PGAtomStorage::TVID(const TruthValue &tv)
{
...
TruthValue* PGAtomStorage::getTV(int tvid)
{
 ...
 */
int PGAtomStorage::do_store_atom(AtomPtr atom)
{
	LinkPtr l(LinkCast(atom));
	if (NULL == l)
	{
 ...
 */
void PGAtomStorage::storeSingleAtom(AtomPtr atom)
{
	get_ids();
...
void PGAtomStorage::do_store_single_atom(AtomPtr atom, int aheight)
{
	setup_typemap();
 ...
 */
void PGAtomStorage::setup_typemap(void)
{
...
void PGAtomStorage::set_typemap(int dbval, const char * tname)
{
...
#ifndef USE_INLINE_EDGES
void PGAtomStorage::getOutgoing(std::vector<Handle> &outv, Handle h)
{
	char buff[BUFSZ];
	UUID uuid = h->getUUID();
...
PGAtomStorage::PseudoPtr PGAtomStorage::getAtom(const char * query, int height)
{
	ODBCConnection* db_conn = get_conn();
	Response rp;
...
PGAtomStorage::PseudoPtr PGAtomStorage::petAtom(UUID uuid)
{
	setup_typemap();
	char buff[BUFSZ];
 ...
 * This method does *not* register the atom with any atomtable/atomspace
 */
LinkPtr PGAtomStorage::getLink(Type t, const HandleSeq& oset)
{
	setup_typemap();
 ...
 */
PGAtomStorage::PseudoPtr PGAtomStorage::makeAtom(Response &rp, UUID uuid)
{
...
#endif
		logger().debug("PGAtomStorage::loadType: Loaded %lu atoms of type %d at height %d\n",
			load_count - cur, db_atom_type, hei);
	}
	put_conn(db_conn);
	logger().debug("PGAtomStorage::loadType: Finished loading %lu atoms in total\n",
		(unsigned long) load_count);
...
bool PGAtomStorage::store_cb(AtomPtr atom)
{
...
void PGAtomStorage::setMaxHeight(int sqmax)
{
...
int PGAtomStorage::getMaxHeight(void)
{
	ODBCConnection* db_conn = get_conn();
	Response rp;
...
UUID PGAtomStorage::getMaxObservedUUID(void)
{
	ODBCConnection* db_conn = get_conn();
	Response rp;
	rp.intval = 0;
...
int PGAtomStorage::getMaxObservedHeight(void)
{
	ODBCConnection* db_conn = get_conn();
	Response rp;
	rp.intval = 0;
...
void PGAtomStorage::reserve(void)
{
