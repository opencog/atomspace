...
{
    const EvidenceCountTruthValue *stv = dynamic_cast<const EvidenceCountTruthValue *>(&rhs);
    if (NULL == stv) return false;

#define FLOAT_ACCEPTABLE_ERROR 0.000001
    return isWithin(_pos_count, stv->_pos_count, FLOAT_ACCEPTABLE_ERROR)
	    and isWithin(_total_count, stv->_total_count, FLOAT_ACCEPTABLE_ERROR);
