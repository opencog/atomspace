...
FUNCTION(COMPILE_MODULE MODULE_NAME)
    ...
    # Add a dependency on the base atomspace module, by default.
  ...
  IF(HAVE_GUILE)
    ...
    IF(DEFINED SCM_FILES)
        ...
        IF (${SCM_COMPILE})
            ...
            # Override GO_INSTALL_PATH to match the .scm install location.
            # Replace GUILE_SITE_DIR with GUILE_CCACHE_DIR in the destination.
            STRING(REPLACE "${GUILE_SITE_DIR}" "${GUILE_CCACHE_DIR}"
                   GO_INSTALL_PATH "${SCM_MODULE_DESTINATION}")
            ...
            # Touch the .go file after .scm files are copied, so that
            # the .go file has a newer timestamp than the .scm files.
            # This prevents Guile from auto-recompiling the module.
            SET(GO_BUILD_OUTPUT "${GUILE_BIN_DIR}/${SCM_MODULE}.go")
            STRING(REPLACE "/" "_" GO_TARGET_NAME ${SCM_MODULE})
...
if(${CMAKE_VERSION} VERSION_GREATER "3.11.0")
        ...
        #
        # Note: We only compile the module file (the first file in the
        # list). Auxiliary files are NOT compiled separately, because
        # they are typically include-from-path'd by the module file,
        # meaning their content is already compiled into the module's
        # .go bytecode file.
        FOREACH(FILE_PATH ${SCM_FILES})
        ...
        # Install the compiled module .go file, and touch it so that
        # the timestamp on the module.go file is newer than the
        # timestamp on the module.scm file.
        IF (${SCM_COMPILE})
            GET_FILENAME_COMPONENT(GO_FILE_NAME ${SCM_MODULE} NAME)
            SET(GO_FILE_NAME "${GO_FILE_NAME}.go")
            INSTALL(
                FILES ${GUILE_BIN_DIR}/${SCM_MODULE}.go
                DESTINATION ${GO_INSTALL_PATH})
            INSTALL(CODE "
                FILE(TOUCH \"\$ENV{DESTDIR}${GO_INSTALL_PATH}/${GO_FILE_NAME}\")
            ")
        ENDIF()
...
FUNCTION(WRITE_GUILE_CONFIG OUTPUT_FILE CONFIG_TARGET SCM_IN_BUILD_DIR)
    ...
    foreach(PATH_PAIR ${SYMBOL_PATH_LIST})
        ...
        ELSE (SCM_IN_BUILD_DIR)
            FILE(APPEND "${OUTPUT_FILE}"
                "(define-public ${SYMBOL} \"${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_LIBDIR}/opencog/\")\n")
