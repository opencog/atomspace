...
class LinkValueContainingAtomsTest(unittest.TestCase):
    ...
    def test_create_list_value(self):
        value = LinkValue([FloatSeqValue(1), StringSeqValue('foo'),
                           ConceptNode('bar')])
        self.assertTrue(value is not None)
    ...
    def test_value_equals(self):
        ...
        self.assertEqual(LinkValue([FloatSeqValue(1), StringSeqValue('foo'),
                                    ConceptNode('bar')]),
                         LinkValue([FloatSeqValue(1), StringSeqValue('foo'),
                                    ConceptNode('bar')]))
    ...
    def test_add_value_to_atom(self):
        ...
        value = LinkValue([StringSeqValue('a'), FloatSeqValue(1),
                           ConceptNode('bar')])
        atom.set_value(key, value)
        self.assertEqual(LinkValue([StringSeqValue('a'), FloatSeqValue(1),
                                    ConceptNode('bar')]),
                         atom.get_value(key))
