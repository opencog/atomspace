...
class UnisetValueMultithreadTest(unittest.TestCase):
    ...
    def test_concurrent_add_duplicates(self):
        """Test that concurrent adds of duplicates work correctly."""
        uniset = UnisetValue()
        uniset.open()
        ...
        def adder(thread_id):
            """Add the same values repeatedly."""
            nonlocal add_count
            try:
                for i in range(1000):
                    # All threads add the same 10 values
                    for j in range(10):
                        uniset.add(FloatValue(float(j)))
                        with add_lock:
                            add_count += 1
        ...
        # Start multiple threads adding the same values
        threads = []
        for i in range(5):
        ...
        # Despite many adds, should only have 10 unique values
